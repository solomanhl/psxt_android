	...
	public class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("create table IF NOT EXISTS " + table_people + 
					...
					"f2 varchar(3), " +	//分数2
					"f3 varchar(3), " +  //分数3	
					"xiaozufen varchar(3), " +	//小组评分，现场分组小组意见时候用
					"xiaozuyijian varchar(6) )");	//小组意见，现场分组小组意见时候用
	...
	public Cursor queryTable(String id) {
		...
		try {
			cursor = SQLdb.query(table_people, // table名
					new String[] { "id", "pinfen", "pogeyijian", "poge", "toupiao","tijiaostate" ,"f1","f2","f3", "xiaozufen", "xiaozuyijian"}, // 字段
					"id = '" + id + "'", // 条件
					null, null, null, null);
		} catch (Exception e) {
			e.printStackTrace();
